(function () {
  ...
  function addKeywords(source, keywords) {
    ...
    let kStr = '';
    keywords.forEach(k => { kStr += '#define ' + k + '\n'; });
    return kStr + source;
  }
  ...
  function hexToRGB(hex) {
    ...
    return {
      r: parseInt(val.slice(0, 2), 16) / 255 * 0.35,
      g: parseInt(val.slice(2, 4), 16) / 255 * 0.35,
      b: parseInt(val.slice(4, 6), 16) / 255 * 0.35
    };
  }
  ...
  // Reduce fluid intensity to 20% when hovering over the nav/header
  (function () {
    const NAV_FORCE  = config.SPLAT_FORCE  * 0.05;
    const NAV_RADIUS = config.SPLAT_RADIUS * 0.05;
    const NAV_BASE_FORCE  = config.SPLAT_FORCE;
    const NAV_BASE_RADIUS = config.SPLAT_RADIUS;

    const navEl = document.getElementById('nav');
    if (navEl) {
      navEl.addEventListener('mouseenter', function () {
        config.SPLAT_FORCE  = NAV_FORCE;
        config.SPLAT_RADIUS = NAV_RADIUS;
      });
      navEl.addEventListener('mouseleave', function () {
        config.SPLAT_FORCE  = NAV_BASE_FORCE;
        config.SPLAT_RADIUS = NAV_BASE_RADIUS;
      });
    }
  })();
